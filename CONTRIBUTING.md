...
- Core pieces:
  ...
  - `packages/desktop-electron`: The native desktop app in this fork, built with Electron (wraps `packages/app`)
...
This fork focuses on the Electron desktop app in `packages/desktop-electron`.

To run the desktop app from the repo root:
...
Renderer changes use Vite HMR. Main and preload changes rebuild and restart Electron.
...
To package the desktop app for the current platform:
